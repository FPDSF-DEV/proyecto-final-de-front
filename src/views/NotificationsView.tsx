        ...
        <div className="flex-1 flex flex-col gap-2 overflow-y-auto scrollbar-hide pr-1">
          {filteredNotifs.map(n => (
            <div 
              ...
              className={`bg-[var(--bg)] rounded-[var(--radius-md)] p-3.5 border border-[var(--border-light)] cursor-pointer transition-all flex gap-3 items-start shrink-0 group ${n.id === selectedId ? 'border-[var(--primary)] bg-white shadow-sm ring-2 ring-[var(--primary-light)]' : 'hover:border-[var(--border-strong)] hover:bg-white'} ${n.unread ? 'border-l-4 border-l-[var(--primary)]' : ''} ${n.urgent ? 'border-l-4 border-l-[var(--primary)] bg-[var(--primary-light)]/20' : ''}`}
            >
              <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 font-bold text-[0.8rem] transition-transform group-hover:scale-105 ${n.type === 'danger' ? 'bg-[var(--primary-light)] text-[var(--primary)]' : n.type === 'warning' ? 'bg-[var(--primary-light)] text-[var(--primary)]' : n.type === 'success' ? 'bg-[var(--primary-light)] text-[var(--primary)]' : 'bg-[var(--primary-light)] text-[var(--primary)]'}`}>
            ...
            {selectedNotif ? (
              ...
              >
                <div className="flex items-start gap-4 p-1">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 font-bold text-[1.1rem] shadow-sm ${selectedNotif.type === 'danger' ? 'bg-[var(--primary-light)] text-[var(--primary)]' : selectedNotif.type === 'warning' ? 'bg-[var(--primary-light)] text-[var(--primary)]' : selectedNotif.type === 'success' ? 'bg-[var(--primary-light)] text-[var(--primary)]' : 'bg-[var(--primary-light)] text-[var(--primary)]'}`}>
                ...
                <div className="flex gap-3 flex-wrap shrink-0">
                  <button className="px-5 py-2.5 rounded-full bg-[var(--primary)] text-white font-bold text-[0.7rem] flex items-center gap-2 hover:bg-[var(--primary-dark)] transition-all shadow-md active:scale-95">
...
function FilterPill({ active, onClick, children }: { active: boolean, onClick: () => void, children: React.ReactNode }) {
  return (
    <button 
      onClick={onClick}
      className={`text-[0.62rem] font-bold px-3 py-1.5 rounded-full border-2 transition-all active:scale-95 ${active ? 'bg-[var(--primary)] text-white border-[var(--primary)] shadow-sm' : 'bg-transparent text-[var(--text-muted)] border-[var(--border)] hover:border-[var(--border-strong)] hover:text-[var(--primary)]'}`}>
      {children}
    </button>
  );
}