      ...
      <div className="block-card flex flex-col overflow-hidden">
        <div className="flex items-center justify-between mb-4 shrink-0 flex-wrap gap-2">
          <div className="flex items-center gap-3">
            <button 
              onClick={prevMonth}
              className="w-8 h-8 rounded-full border border-[var(--border)] flex items-center justify-center hover:border-[var(--border-strong)] hover:bg-[var(--primary-light)] transition-all active:scale-90"
            >
              <ChevronLeft size={16} />
            </button>
          ...
          <div className="flex gap-4 text-[0.62rem] font-bold text-[var(--text-muted)] bg-gray-50 px-3 py-1.5 rounded-full border border-gray-100">
            <span className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-[var(--primary)] shadow-[0_0_5px_var(--primary-glow)]"></span> Reportada
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-[var(--primary-light)] shadow-[0_0_5px_var(--primary-glow)]"></span> Por reportar
            </span>
          </div>
        </div>
                ...
                >
                  ...
                  {(hasR || hasU) && (
                    <div className="flex gap-1 mt-1">
                      {hasR && <div className={`w-1.5 h-1.5 rounded-full ${isSelected ? 'bg-white' : 'bg-[var(--primary)] shadow-[0_0_4px_var(--primary-glow)]'}`} />}
                      {hasU && <div className={`w-1.5 h-1.5 rounded-full ${isSelected ? 'bg-white' : 'bg-[var(--primary-light)] shadow-[0_0_4px_var(--primary-glow)]'}`} />}
                    </div>
                  )}
                  {isToday && !isSelected && (
      ...
      <div className="block-card flex flex-col min-h-0 overflow-hidden bg-white/50 backdrop-blur-sm">
        <div className="flex items-center justify-between mb-4 shrink-0 flex-wrap gap-2">
          ...
          <div className="flex gap-1.5">
            {uCount > 0 && <span className="text-[0.55rem] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-[var(--primary-light)] text-[var(--primary)] border border-[var(--border)]">{uCount} pendientes</span>}
            {rCount > 0 && <span className="text-[0.55rem] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-[var(--primary-light)] text-[var(--primary)] border border-[var(--border)]">{rCount} reportadas</span>}
          </div>
        </div>
            ...
            {activities.length > 0 ? (
              ...
              >
                {activities.map((a, i) => (
                  <div 
                    key={i}
                    className={`bg-white rounded-[var(--radius-lg)] p-4 border border-[var(--border)] hover:border-[var(--border-strong)] hover:shadow-md transition-all flex flex-col gap-2 shrink-0 group ${a.reported ? 'border-l-4 border-l-[var(--primary)]' : 'border-l-4 border-l-[var(--border)]'}`}
                  >
                    <div className="flex justify-between items-start">
                      <div className="text-[0.6rem] font-black uppercase tracking-wider text-[var(--text-muted)] opacity-70">{a.course}</div>
                      <div className={`px-2 py-0.5 rounded-md text-[0.55rem] font-bold bg-[var(--primary-light)] text-[var(--primary)]`}>
                        {a.type}
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      <button className="px-4 py-1.5 rounded-full bg-[var(--primary)] text-white font-bold text-[0.65rem] flex items-center gap-1.5 hover:bg-[var(--primary-dark)] transition-all shadow-sm active:scale-95">
